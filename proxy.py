...
# - Exposes /health as a vLLM-compatible aggregate health endpoint.
# - Exposes /metrics by merging backend Prometheus metrics with role/instance labels.
#
# Prerequisites:
# - Python 3.10+
...
#
# For testing, you can use actual vLLM servers:
#
...
#       -H "Content-Type: application/json" \
#       -d '{"model":"your-model","messages":[{"role":"user","content":"Hello!"}],"max_tokens":16}'
#
# Step 4: Health Check
# --------------------
#
...
#
# /health returns only the aggregate proxy availability:
# - HTTP 200: at least one prefiller and at least one decoder are healthy.
# - HTTP 503: otherwise.
#
# The old /healthcheck endpoint is preserved and still returns instance counts.
#
# Step 5: Metrics
# ---------------
#
...
#
# /metrics fetches each backend's root /metrics endpoint, injects labels
# role="prefill|decode" and instance="host:port" into every sample line, and
# returns one merged Prometheus text response.
#
# Step 6: Add or Remove Prefiller or Decoder Instances (Optional)
# ---------------------------------------------------------------
#
#   curl -X POST http://localhost:9000/instances/add \
#       -H "Content-Type: application/json" \
#       -d '{"type":"prefill","instances":["127.0.0.1:8102","127.0.0.1:8103"]}'
#
#   curl -X POST http://localhost:9000/instances/remove \
#       -H "Content-Type: application/json" \
#       -d '{"type":"decode","instances":"127.0.0.1:8201"}'
...
class ProxyState:
    def __init__(self, prefiller_instances, decoder_instances):
        ...
        self.prefiller_heap = [
            (0.0, i, server) for i, server in enumerate(self.prefillers)
        ]
    ...
    def abort_prefiller_request(self, server_idx: int, request_id):
        ...
        This helps release kv cache in prefiller node.
        """
    ...
    def add_prefillers(self, instances: list[ServerState]) -> None:
        for server in instances:
            if server in self.tainted_prefillers:
                ...
                self.prefiller_heap.append((0, len(self.prefillers) - 1, server))
                heapq.heapify(self.prefiller_heap)
    ...
    def add_decoders(self, instances: list[ServerState]) -> None:
        for server in instances:
            if server in self.tainted_decoders:
                ...
                self.decoder_heap.append((0, len(self.decoders) - 1, server))
                heapq.heapify(self.decoder_heap)
    ...
    def remove_prefillers(self, instances: list[ServerState]) -> bool:
        ...
        prefiller_heap_copy.sort(key=lambda x: x[1])
        prefiller_heap = []
        idx = 0
    ...
    def remove_decoders(self, instances: list[ServerState]) -> bool:
        ...
        decoder_heap_copy.sort(key=lambda x: x[1])
        decoder_heap = []
        idx = 0
    ...
    def _taint_prefillers(self, instances: list[ServerState]) -> None:
        ...
        self.prefiller_heap = [
            (TAINT_PRIORITY, idx, srv)
            if srv in instances_to_taint
            else (priority, idx, srv)
            for priority, idx, srv in self.prefiller_heap
        ]
        heapq.heapify(self.prefiller_heap)
...
class NodeListener:
    ...
    def _node_listener(self) -> None:
        while True:
            ...
            if self.proxy_state.tainted_prefillers and not self.proxy_state.request_num:
                need_waiting = self.proxy_state.remove_prefillers(
                    self.proxy_state.tainted_prefillers
                )
                if not need_waiting:
                    self.proxy_state.tainted_prefillers.clear()

            if self.proxy_state.tainted_decoders and not self.proxy_state.request_num:
                need_waiting = self.proxy_state.remove_decoders(
                    self.proxy_state.tainted_decoders
                )
                if not need_waiting:
    ...
    async def check_instance_status(client: httpx.AsyncClient) -> bool:
        # vLLM OpenAI server exposes /health at the root path.
        # Because client.base_url is http://host:port/v1, absolute path
        # "/health" is resolved by httpx to http://host:port/health.
        endpoint = "/health"
...
def parse_args():
    ...
    parser.add_argument(
        "--max-retries", type=int, default=3, help="Maximum number of retries for HTTP requests"
    )
    parser.add_argument(
        "--retry-delay",
        type=float,
...
async def lifespan(app: FastAPI):
    ...
    print(
        f"Initialized {len(proxy_state.prefillers)} prefill clients "
...
async def _handle_completions(api: str, request: Request):
    try:
        ...
        # Refer to vLLM sampling_params: max_tokens default value.
        ...
        async def generate_stream():
            ...
            try:
                ...
                logger.error(
                    ...
                    "will be routing to the target prefiller when new request is ready "
                    "to dispatch to it",
